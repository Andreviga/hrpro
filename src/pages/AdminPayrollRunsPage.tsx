/**
 * Pagina administrativa para abertura, fechamento e reabertura de folhas por competencia.
 */
  ...
  const loadRuns = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getFriendlyError(error, 'Nao foi possivel carregar as folhas.')
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenRun = async () => {
    if (!openForm.month || !openForm.year) {
      toast({
        ...
        description: 'Informe mes e ano para abrir a competencia.'
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: getFriendlyError(error, 'Nao foi possivel abrir a competencia.')
      });
    }
  };
  ...
  const handleLoadSummary = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getFriendlyError(error, 'Nao foi possivel obter o resumo.')
      });
    } finally {
  ...
  const confirmAction = async () => {
    ...
    try {
      if (actionType === 'close') {
        ...
        toast({
          title: 'Folha fechada',
          description: `Competencia ${actionRun.month}/${actionRun.year} fechada com sucesso.`
        });
      } else {
        await payrollApi.reopenRun(actionRun.id);
        toast({
          title: 'Folha reaberta',
          description: `Competencia ${actionRun.month}/${actionRun.year} reaberta.`
        });
      }
      await loadRuns();
    } catch (error) {
      toast({
        ...
        description: getFriendlyError(error, 'Nao foi possivel concluir a operacao.')
      });
    } finally {
      setDialogOpen(false);
    }
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Fechamento por Competencia</h1>
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                ...
                <span>Resumo por competencia</span>
              </CardTitle>
      ...
      <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {actionRun
                ? `Competencia ${actionRun.month}/${actionRun.year}. Deseja continuar?`
                : 'Deseja continuar?'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
